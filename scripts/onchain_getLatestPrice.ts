...
async function makeDeploy() {
  ...
  const MyContract = await ethers.getContractFactory("PriceFeedAggregatorV3", wallet); //指定账户部署，需要用私钥初始化
  ...
  console.log("\n", "获取交易价格...");
  const [roundID, price, startedAt, timeStamp, answeredInRound] = await myContract.getLatestPrice();
